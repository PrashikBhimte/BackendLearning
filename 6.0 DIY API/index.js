...
app.get("/filter", (req, res) => {
  ...
  const data = jokes.filter((joke) => joke.jokeType === type );
  res.json(data)
})
...
app.put("/jokes/:id", (req, res) => {
  ...
  const indexJoke = jokes.findIndex((joke) => joke.id === id);
  jokes[indexJoke] = joke;
  res.json(joke);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  indexJoke = jokes.findIndex(existingJoke);
  jokes[indexJoke] = joke;
  res.json(joke);
});
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const indexJoke = jokes.findIndex((joke) => joke.id === id);
  if (indexJoke < 0) {
    res.send(`Error : No joke found of id ${id}.`);
  }
  else{
    jokes.slice(indexJoke, 1);
    res.sendStatus(200);
  }
});
